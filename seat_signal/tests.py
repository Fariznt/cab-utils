...
class PollCircuitBreakerTests(TestCase):
    """
    A persistent C@B failure (a response-format change, say) would otherwise
    log an error every poll cycle forever, and each one is a billed alert SMS
    once CloudWatch alerting is wired up. Past POLL_ERROR_LIMIT the loop exits
    for systemd to restart it, alerting once instead of on a loop.
    """

    def setUp(self):
        # The counter is module-level, so it outlives any single test.
        services._consecutive_failures = 0
        self.user = User.objects.create_user(phone_num="+15551112222")
        self.session = CourseSession.objects.create(
            crn="12345", department_code="CSCI", course_code="0150",
            section="S01", sem_id="202410", title="Test",
        )
        create_watch(self.user, self.session)

    @override_settings(POLL_ERROR_LIMIT=3)
    @patch("seat_signal.services.check_seat_availability", side_effect=RuntimeError("C@B changed"))
    def test_exits_once_failures_hit_the_limit(self, mock_check):
        # One watched session, so one failure per pass.
        list(find_signals_with_open_seats())
        list(find_signals_with_open_seats())
        with self.assertRaises(SystemExit):
            list(find_signals_with_open_seats())

        self.assertTrue(EventLog.objects.filter(level="CRITICAL").exists())

    @override_settings(POLL_ERROR_LIMIT=3)
    def test_a_successful_check_resets_the_count(self):
        # Intermittent failures must not accumulate into a false shutdown.
